...
public abstract class AbstractListProcessorViaS <T extends ListableEntityViaS> extends AbstractProcessor{
    public static final PropertyDescriptor DISTRIBUTED_CACHE_SERVICE = new PropertyDescriptor.Builder()
        ...
        private volatile ListedEntityTrackerViaS<T> listedEntityTrackerViaS;
        ...
        protected final Collection<ValidationResult> customValidate(ValidationContext context) {
            ...
            if (BY_ENTITIES.equals(listingStrategy)) {
                ListedEntityTrackerViaS.validateProperties(context, results, getStateScope(context));
            }
        ...
        public void initListedEntityTracker(ProcessContext context) {
            ...
            if (listedEntityTrackerViaS != null && (resetEntityTrackingState || !isTrackingEntityStrategy)) {
                try {
                    listedEntityTrackerViaS.clearListedEntities();
                } catch (IOException e) {
            ...
            if (isTrackingEntityStrategy) {
                if (listedEntityTrackerViaS == null) {
                    listedEntityTrackerViaS = createListedEntityTracker();
                }
            } else {
                listedEntityTrackerViaS = null;
            }
        }

        protected ListedEntityTrackerViaS<T> createListedEntityTracker() {
            return new ListedEntityTrackerViaS<>(getIdentifier(), getLogger());
        }

        private void listByTrackingEntities(ProcessContext context, ProcessSession session) throws ProcessException {
            listedEntityTrackerViaS.trackEntities(context, session, justElectedPrimaryNode, getStateScope(context), minTimestampToList -> {
                try {
